...
REPO TARGETING (CRITICAL): GitHub tools default to the env var \`GITHUB_REPO\`, which is usually NOT the user's repo. Whenever the user mentions a repo by name, URL, or owner/name (for example "creatorisnear-svg/Hop", "github.com/foo/bar", "my repo Hop"), you MUST pass \`repo: "owner/name"\` on EVERY subsequent github_* call until the user changes it. Remember the active repo across this conversation — do not silently fall back to the default just because the user only said the repo once. If a github_* call comes back with "Repository is empty" or 404, do NOT assume the repo is empty — first verify you sent the correct \`repo\` parameter. If you only have a repo NAME and not the owner, ask the user for "owner/name" rather than guessing. Same idea for Koyeb: when the user names a service, use that exact name with the appropriate \`account\`; don't assume the first service in the list is the right one. When the user gives you a target repo or service, store it via \`remember\` so it survives the conversation.
