...
export const marketWatchesTable = pgTable(
  "market_watches",
  {
  ...
  (t) => ({
    bySymbol: index("market_watches_symbol_idx").on(t.symbol),
  }),
);

export const marketPredictionsTable = pgTable(
  "market_predictions",
  {
    id: text("id").primaryKey(),
  ...
  (t) => ({
    // Hot path: load last N predictions for a watch (track record block,
    // history list). This composite index lets Postgres serve both the
    // filter and the ordering from a single index walk.
    byWatchTime: index("market_predictions_watch_created_idx").on(
      t.watchId,
      t.createdAt.desc(),
    ),
  }),
...
export const marketUserTradesTable = pgTable(
  "market_user_trades",
  {
    id: text("id").primaryKey(),
    watchId: text("watch_id").notNull(),
  ...
  (t) => ({
    // Hot path: trades panel polls every 5s with `where watch_id=? order by opened_at desc`.
    byWatchOpened: index("market_user_trades_watch_opened_idx").on(
      t.watchId,
      t.openedAt.desc(),
    ),
    // Hot path: enrichment / open-trade scans across all watches.
    byStatus: index("market_user_trades_status_idx").on(t.status),
  }),
);
